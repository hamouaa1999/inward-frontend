...
import { Text, View, Button, TextInput, TouchableOpacity, BackHandler } from 'react-native';
...
export default function Signup({ navigation }) {
    ...
    const signup = () => {
        if (fullname.length > 0 && username.length > 0 && password.length > 0) {
            axios.post('http://' + SERVER_ADDRESS + ':5000/api/auth/signup', {
                fullName: fullname,
                username: username,
                password: password
            }).then((response) => {
    ...
    return (
        <View>
            ...
            <View className="flex-row border-solid border-2 rounded-full mx-10 mb-2">
                ...
                <TouchableOpacity>
                {
                    showPassword ? (<Icon.Eye onPress={() => setShowPassword(!showPassword)} className="mt-3 ml-0" height="20" width="20" stroke="black" />) 
